...
class BaselineDasymetric(nn.Module):
    ...
    def __init__(self, lambda_lights=0.01, lambda_settlement=0.01, eps=1e-8, baseline_floor=1e-3):
        """
        Args:
            ...
            baseline_floor: Minimum baseline used in residualization/reconstruction
        """
        super().__init__()
    ...
    def compute_residual_target(self, population, lights, settlement,
        ...
        baseline_eff = torch.clamp(baseline, min=self.baseline_floor)

        # Compute log-residual
        # R[i] = log((Y[i] + ε) / (B[i] + ε))
        residual = torch.log((population + eps) / (baseline_eff + eps))

        return residual, baseline_eff
    ...
    def reconstruct_from_residual(self, residual, lights, settlement,
        ...
        population = baseline_eff * torch.exp(residual)
