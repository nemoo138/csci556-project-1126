...
class DualBranchConditionalEncoder(nn.Module):
    ...
    def __init__(self, cond_channels=256, low_res_ch=128, high_res_ch=128,
                 low_in_channels=1, high_in_channels=1):
        """
        Args:
            ...
            low_in_channels: Input channels for low-res branch (lights [+ mask])
            high_in_channels: Input channels for high-res branch (settlement [+ mask])
        """
        super().__init__()
        ...
        self.low_res_branch = nn.Sequential(
            # 256 → 128
            nn.Conv2d(low_in_channels, 32, kernel_size=5, stride=2, padding=2),
            nn.GroupNorm(4, 32),
            nn.SiLU(),
        ...
        self.high_res_branch = nn.Sequential(
            # 256 → 256
            nn.Conv2d(high_in_channels, 64, kernel_size=3, stride=1, padding=1),
            nn.GroupNorm(8, 64),
            nn.SiLU(),
    ...
    def forward(self, lights, settlement):
        ...
        Args:
            lights: (B, 1 or 2, 256, 256) VIIRS nighttime lights (+ optional mask)
            settlement: (B, 1 or 2, 256, 256) World Settlement Footprint (+ optional mask)
