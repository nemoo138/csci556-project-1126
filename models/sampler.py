...
def soft_clamp(x, min_val=-10.0, max_val=10.0):
    """Soft clamp used in inference to match training behavior."""
...
class C3LDMSampler:
    ...
    def sample_population_map(
        ...
        Args:
            ...
            lights_mask: (B, 1, 256, 256) missingness mask for lights (optional)
            settlement_mask: (B, 1, 256, 256) missingness mask for settlement (optional)
        ...
        baseline_floor = getattr(self.baseline, 'baseline_floor', 1e-3)
        baseline_pop = torch.clamp(baseline_pop, min=baseline_floor)

        # 2. Conditioning
        # Spatial conditioning
        use_mask = getattr(self.cond_encoder, 'low_in_channels', 1) > 1
        if use_mask:
        ...
        H_cond_spatial = self.cond_encoder(lights_cond, settlement_cond)
        ...
        for sample_idx in range(num_samples):
            ...
            # NEW: soft clamp to keep gradient
            residual = soft_clamp(residual, min_val=-10.0, max_val=10.0)

            #  ADD THIS DIAGNOSTIC:
            # print(f"\n=== INFERENCE Diagnostics ===")
            # print(f"z_0 range: [{z_0.min():.3f}, {z_0.max():.3f}], mean={z_0.mean():.3f}, std={z_0.std():.3f}")
            # print(f"Residual range: [{residual.min():.3f}, {residual.max():.3f}], mean={residual.mean():.3f}")
            # print(f"Baseline range: [{baseline_pop.min():.6f}, {baseline_pop.max():.6f}], sum={baseline_pop.sum():.2f}")
            # print(f"exp(residual) range: [{torch.exp(residual).min():.4f}, {torch.exp(residual).max():.4f}]")


            # 5. Convert to population: P = B * exp(R)
            pop_raw = baseline_pop * torch.exp(residual)
            pop_raw = torch.clamp(pop_raw, min=0)
            print(f"pop_raw range: [{pop_raw.min():.4f}, {pop_raw.max():.4f}], sum={pop_raw.sum():.2f}")
