...
def load_checkpoint(
    ...
    # MODIFIED: make RNG restore safe instead of hard-crashing on wrong types
    if 'random_states' in checkpoint and isinstance(checkpoint['random_states'], dict):
        rs = checkpoint['random_states']
        ...
        if torch_state is not None:
            try:
                ...
                print(f"  Warning: Failed to restore torch RNG state ({type(torch_state)}): {e}")
                torch_restored = False
        else:
            torch_restored = False
        ...
        if cuda_state is not None and torch.cuda.is_available():
            try:
                if not isinstance(cuda_state, torch.Tensor):
                    cuda_state = torch.tensor(cuda_state, dtype=torch.uint8, device='cuda')
                else:
                    if cuda_state.dtype != torch.uint8:
                        cuda_state = cuda_state.to(dtype=torch.uint8)
                    if not cuda_state.is_cuda:
                        cuda_state = cuda_state.to('cuda')

                torch.cuda.set_rng_state(cuda_state)
                cuda_restored = True
            except Exception as e:
                print(f"  Warning: Failed to restore CUDA RNG state ({type(cuda_state)}): {e}")

        if torch_restored or cuda_restored:
            print("  Restored random states "
                  f"(torch={torch_restored}, cuda={cuda_restored})")
        else:
            print("  Skipped RNG state restore (missing or incompatible format).")
